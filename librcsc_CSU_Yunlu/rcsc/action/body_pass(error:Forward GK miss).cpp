...
*/
bool
Body_Pass::execute_through( PlayerAgent * agent )
{
    dlog.addText( Logger::ACTION,
                  "%s:%d: Body_Pass. execute()"
                  ,__FILE__, __LINE__ );
    ...
    Vector2D target_point(36.0, 0.0);   //50->36
    double first_speed = 0.0;
    int receiver = 0;

    if ( ! get_best_through_pass( agent->world(), &target_point, &first_speed, &receiver ) )
    {
        return false;
    }
...
//                      S_last_calc_speed, S_last_calc_receiver );
    ...
    {
        //goalie refused
        std::cerr << world.time().cycle() << __FILE__ << ": DO NOT PASS TO GOALIE !!! \n\n";
//        return false;
    }
...
bool
Body_Pass::get_best_through_pass( const WorldModel & world,
                          Vector2D * target_point,
                          double * first_speed,
                          int * receiver )
{
    ...
    // create route
    create_through_pass_routes( world );
...
Body_Pass::create_routes( const WorldModel & world )
{
          ...
          ++it )
    {
        ...
        if ( world.self().pos().x > 35.0 && world.self().pos().absY() < 17.0 )
        {
            // no lead_pass
            std::cerr << world.time().cycle() << __FILE__ << world.self().unum() << ": no lead_pass? \n\n";
        }
        else
        {
        ...
        if ( world.self().pos().x > world.offsideLineX() - 30.0 )    //20.0->30.0               //??????????????????
        {
...
*/
void
Body_Pass::create_through_pass_routes( const WorldModel & world )
{
    // reset old info
    S_cached_pass_route.clear();
          ...
          ++it )
    {
        ...
        if ( (*it)->pos().x > world.offsideLineX() + 0.1 )  //1->0?
        {
            // offside players are rejected.
            continue;
        }
        if ( (*it)->pos().x < world.ball().pos().x - 5.0 )  //25->5
        {
            // too back
            continue;
        }
...
//        create_direct_pass( world, *it );
//        create_lead_pass( world, *it );
        if ( world.self().pos().x > world.offsideLineX() - 45.0 )                   //??????????????????
        {
            create_through_pass( world, *it );
        }
    }
                               ...
                               const PlayerObject * receiver )
{
    ...
    // out of pitch?
    if ( receiver->pos().absX() > ServerParam::i().pitchHalfLength() - 3.0  //3->1??
    ...
    // too close
    if ( receiver->distFromSelf() < 3.0 )                                                       //need to delet?
    {
#ifdef DEBUG
        dlog.addText( Logger::PASS,
...
//#endif
    ...
    if ( receiver->pos().x < world.self().pos().x + 14.0 )
    {
        if ( receiver->angleFromSelf().abs() < 40.0
             || ( receiver->pos().x > world.ourDefenseLineX() + 10.0
                  && receiver->pos().x > 0.0 )
...
//#endif
...
#if 0
    const PlayerPtrCont & opps = world.opponentsFromSelf();
    for ( PlayerPtrCont::const_iterator it = opps.begin();
          it != opps.end();
          ++it )
    {
        if ( (*it)->pos().dist( base_player_pos ) < 3.0 )
            return;
   ...
   /////////////////////////////////////////////////////////////////////////////////////////////////////////
 	bool safe=true;
	first_speed = ServerParam::i().ballSpeedMax();
	double safe_speed = first_speed;
	static const double player_dash_speed = 1.0;
	const rcsc::Line2D pass_line( world.ball().pos(),receiver->pos() );
	for(double present_speed = first_speed;present_speed>=1.8;present_speed-=0.1)
	{
		Vector2D present_vel = Vector2D::polar2vector( present_speed, receiver_angle );
		const PlayerPtrCont::const_iterator
			  ...
			  ++it )
		{
			...
			int pre_step = Body_Pass::predict_player_reach_cycle( *it,
																		base_player_pos,
																		(*it)->playerTypePtr()->kickableArea(),
			...
			Vector2D first_ball_pos = world.ball().pos();
			const double ball_steps_to_target
							= calc_length_geom_series( present_speed,
													   receiver_dist,
													   ServerParam::i().ballDecay() );
			if(pre_step-1< ball_steps_to_target)
			{
				safe=false;
				break;
			}
			...
			int cycle;
			for(cycle=1;cycle <= ball_steps_to_target;cycle++)
			{
				Vector2D ball_pos = inertia_n_step_point( first_ball_pos,
																	 present_vel,
																	 cycle,
																	 ServerParam::i().ballDecay() );
				const Vector2D opp_rel = (*it)->pos() - ball_pos;
				const double opp_dist = opp_rel.r();
				const AngleDeg opp_angle = opp_rel.th();

				int o_step = Body_Pass::predict_player_reach_cycle( *it,
																	ball_pos,
																	(*it)->playerTypePtr()->kickableArea(),
																	0.0, // penalty distance
																	1, // body count thr
			...
			if(safe == false && present_speed == ServerParam::i().ballSpeedMax())
				return;
		}
		if(!safe)
			break;
		if(safe)
			safe_speed = present_speed;
	}
	first_speed = safe_speed+0.1>ServerParam::i().ballSpeedMax()?safe_speed:safe_speed+0.1;
   /////////////////////////////////////////////////////////////////////////////////////////////////////////
                             ...
                             const PlayerObject * receiver )
{
    ...
    //const                                                                                    change
    Vector2D receiver_rel = receiver->pos() - world.ball().pos();
    const double receiver_dist = receiver_rel.r();
    const AngleDeg receiver_angle = receiver_rel.th();
          ...
          total_add_angle_abs += angle_step_abs, ++count )
    {
        ...
        Vector2D leading_point1 = world.ball().pos()+Vector2D::polar2vector(receiver_dist, receiver_angle - total_add_angle_abs);
        Vector2D leading_point2 = world.ball().pos()+Vector2D::polar2vector(receiver_dist, receiver_angle + total_add_angle_abs);
        Vector2D leading_point;

        /////////////////////////////////////////////////////////////////////////////////////////////////////////
          double first_speed = ServerParam::i().ballSpeedMax();
          //double safe_speed = first_speed;
          double safe_speed;
         // double safe_speed2;
          AngleDeg leading_angle;
          double min_speed
          ...
          for(int i = 0;i < 2; i++)
          {
			   if(i==0)
			   {
				   leading_point=leading_point1;
				   leading_angle = receiver_angle - total_add_angle_abs;
			   }
			   if(i==1)
			   {
				   leading_point=leading_point2;
				   leading_angle = receiver_angle + total_add_angle_abs;
			   }
			   bool safe=true;
			   bool existsafe =false;

				ball_steps_to_target
								   = calc_length_geom_series( min_speed,
																	receiver_dist,
																	ServerParam::i().ballDecay() );
			   ...
			   for(present_speed = first_speed;present_speed>=min_speed;present_speed-=0.3)
               {
					ball_steps_to_target
									   = calc_length_geom_series( present_speed,
																		receiver_dist,
					...
					Vector2D present_vel = Vector2D::polar2vector( present_speed, leading_angle );
    ...
    for ( PlayerPtrCont::const_iterator it = opps.begin();
          ...
          ++it )
    {
        if ( (*it)->pos().dist( target_point ) < 3.0 )
            continue;
                               ...
                               const PlayerObject * receiver)
{
    ...
    if ( receiver->angleFromSelf().abs() > 135.0 )  //135->90?
    {
#ifdef DEBUG
        dlog.addText( Logger::PASS,
    ...
    for ( int i = 0; i < S_angle_loop; ++i, dash_angle += S_angle_inc )
    {
        ...
        for ( int j = 0; j < S_dash_loop; ++j, dash_dist += S_dash_inc )
        {
           ...
           double present_speed;
 		   ...
 		   for(present_speed = first_speed;present_speed>=min_speed;present_speed-=0.1)
 		   {
 				 ball_steps_to_target
 									  = calc_length_geom_series( present_speed,
 																 target_dist,
 				 ...
 				 Vector2D present_vel = Vector2D::polar2vector( present_speed, target_angle );
 				 const PlayerPtrCont::const_iterator o_end = world.opponentsFromSelf().end();
 				 for ( PlayerPtrCont::const_iterator
 						  it = world.opponentsFromSelf().begin();
 					  it != o_end;
 ...
 */
int
Body_Pass::predict_player_reach_cycle( const AbstractPlayerObject * player,
                                           const Vector2D & target_point,
 ...
 */
int
Body_Pass::predict_player_turn_cycle( const rcsc::PlayerType * ptype,
