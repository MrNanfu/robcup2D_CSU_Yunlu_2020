...
void
ShootTable2008::search2( const PlayerAgent * agent )
{
    static GameTime s_time( 0, 0 );
    ...
    goal_l.y += std::min( 1.5,
                          0.6 + goal_l.dist( wm.ball().pos() ) * 0.039 );
    goal_r.y -= std::min( 1.5,
                          0.6 + goal_r.dist( wm.ball().pos() ) * 0.039 );
          ...
          ++i, shot_point.y += dist_step )
    {
        ...
        dlog.addText( Logger::SHOOT,
                      ...
                      shot_point.x, shot_point.y );
#endif
        calculateShotPoint2( wm, shot_point, goalie, nearest_opp );
    }
 ...
 */
void
ShootTable2008::calculateShotPoint2( const WorldModel & wm,
                                    const Vector2D & shot_point,
                                    const PlayerObject * goalie,
                                    const PlayerObject * nearest_opp )
{
    ...
    Vector2D shot_Goalie_rel = shot_point - goalie->pos();
    double shotdistFromGoalie = shot_Goalie_rel.r();

    double shot_dist = shot_rel.r();

    const Sector2D sector( wm.self().pos(),0.5, shot_rel.r(),
                                 shot_angle - 45,
                                 shot_angle + 45 );

    if(shot_point.x<32) return;

    const rcsc::Line2D shot_line( wm.ball().pos(),shot_point );
	...
	Vector2D shot_first_vel = Vector2D::polar2vector( shot_first_speed,shot_angle );

	int pre_step = Body_Pass::predict_player_reach_cycle(  goalie,
														   shot_point,
												   		   goalie->playerTypePtr()->kickableArea(),
														   0.0, // penalty distance
														   1, // body count thr
														   1, // default turn step
														   0, // wait cycle
														   true );
	const double ball_steps_to_shot
					= calc_length_geom_series( shot_first_speed,
											   shot_dist,
											   ServerParam::i().ballDecay() );

	if(pre_step - 1 < ball_steps_to_shot)
		return;
    ...
    {
    	if ( (*it)->posCount() > 10 ) continue;
    	if ( !sector.contains((*it)->pos())) continue;

    	for(int cycle=1;cycle <= ball_steps_to_shot;cycle++)
    	{
    		Vector2D ball_pos = inertia_n_step_point( wm.ball().pos(),
												  shot_first_vel,
												  cycle,
    ...
    Shot shot( shot_point, shot_first_speed, shot_angle );
    shot.score_ = 0.0;
    shot.score_ = sum + 5 * shot_line.dist(goalie->pos());
    M_shots2.push_back( shot );
#endif
    ...
    //const Sector2D sector( wm.self().pos(),1.5, 10.0,
                                 //shot_angle - 15,
                                 //shot_angle + 15 );

    double shot_dist = shot_rel.r();
    if(shot_point.x<36) return;
    if(shot_dist>20) return;
    if(shotdistFromGoalie<5) return;
    //if(wm.countOpponentsIn(sector,10,true)>=2) return;
    ...
    for ( PlayerPtrCont::const_iterator
    			  ...
    			  ++it )
    		{
    			...
    			if ( (*it)->posCount() > 10 ) continue;
    			if(shot_line.dist((*it)->pos()) <=3.0 &&
    					(*it)->pos().x >= wm.ball().pos().x+1.5 )
    					return;
    			if((*it)->pos().x >= wm.ball().pos().x)
    				score1=10*shot_line.dist((*it)->pos());
	...
	double score2=0.0;
	score2 = 20*(pre_step-ball_steps_to_shot);
#endif

    Shot shot( shot_point, shot_first_speed, shot_angle );
    shot.score_ = 0.0;
    shot.score_ = score1 + 10*shotdistFromGoalie;
    M_shots2.push_back( shot );
}
}
/*-------------------------------------------------------------------*/
