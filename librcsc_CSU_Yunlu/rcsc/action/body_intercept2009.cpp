...
Body_Intercept2009::execute( PlayerAgent * agent )
{
    ...
    if ( table->selfReachCycle() > 100 )
    {
        ...
        std::cerr << agent->world().time().cycle() << __FILE__ << ": intercept here (no solution... Just go to ball end point)  " << agent->world().self().unum() << std::endl;
...
Body_Intercept2009::doKickableOpponentCheck( PlayerAgent * agent )
{
         ...
         && wm.existKickableOpponent() )
    {
        if ( wm.self().goalie() )
        {
            std::cerr << wm.time().cycle() << __FILE__ << ": goalie should clear ball here.?.? \n\n";
        }
        ...
        if ( opp )
        {
            ...
            if ( attack_pos.dist2( goal_pos ) + 3.5 > my_next.dist2( goal_pos ) )   //+ 0.5 at 20140812
            {
                dlog.addText( Logger::INTERCEPT,
                ...
                std::cerr << agent->world().time().cycle() << __FILE__ << ": intercept here   " << agent->world().self().unum() << std::endl;
                return true;
            }
        }
    }
