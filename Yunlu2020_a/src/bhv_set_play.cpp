...
void
Bhv_SetPlay::doCornerKickmove(PlayerAgent * agent)
{
	...
	const PlayerPtrCont::const_iterator end = wm.opponentsFromSelf().end();  //遍历队友到球的距离
			...
			++p )
		{
			...
			Vector2D teammate_point = wm.getTeammateNearestTo((*p)->pos(),5,&min_dist)->pos();
			if(self_min_dist>min_dist)  continue;
			else
			{
				Vector2D target_opp_point=(*p)->pos();


				 if ( ( target_opp_point - home_point ).r() < 10)
				 {
				    Vector2D target_opp_rel = target_opp_point - wm.ball().pos();

				 	double targetdistFromball = target_opp_rel.r();
				 	target_point.x = target_opp_point.x - 2 * target_opp_rel.x / targetdistFromball;
				 	target_point.y = target_opp_point.y - 2 * target_opp_rel.y / targetdistFromball;
