...
#define CREATE_SEVERAL_CANDIDATES_ON_SAME_POINT //ht: testing...
                                            ...
                                            const char * description )
{
    ...
    for ( int step = min_step; step <= max_step; ++step )
    {
        ...
        if ( receiver.player_->unum() == 1 )
        {
            std::cerr << __FILE__ << "  line " << __LINE__ << ": DO NOT PASS TO OUR GOALIE !!! \n\n\n";
            break;
        }
...
StrictCheckPassGenerator::getNearestReceiverUnum( const Vector2D & pos )
{
          ...
          ++p )
    {
        ...
        if ( d2 < min_dist2 && p->player_->unum() != 1 )    //ht: testing...
        {
            min_dist2 = d2;
            unum = p->player_->unum();
        }
