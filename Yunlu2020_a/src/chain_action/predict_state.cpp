...
//    M_all_teammates = M_world->teammatesFromSelf();
...
//    for ( PredictPlayerPtrCont::const_iterator it = M_all_teammates.begin();
    const PredictPlayerPtrCont::const_iterator t_end = M_our_players.end();
    for ( PredictPlayerPtrCont::const_iterator it = M_our_players.begin();
          it != t_end;
          ++it )
    {
        if ( (*it)->isValid()
