...
IntentionNormalDribble::checkOpponent( const WorldModel & wm )
{
         ...
         && ball_next.absY() < ServerParam::i().penaltyAreaHalfWidth() )
    {
        ...
        if ( opp_goalie
             && opp_goalie->distFromBall() < ( ServerParam::i().tackleDist() //catchableArea()  ht: 20150705
    ...
    if (  nearest_opp->distFromBall() < ServerParam::i().tackleDist() + 0.2 )//defaultKickableArea() + 0.2 )
    {
        dlog.addText( Logger::DRIBBLE,
    ...
    if ( nearest_opp->pos().dist( ball_next )
         < ServerParam::i().defaultPlayerSpeedMax() + ServerParam::i().tackleDist() + 0.3 ) //tackledist
    {
